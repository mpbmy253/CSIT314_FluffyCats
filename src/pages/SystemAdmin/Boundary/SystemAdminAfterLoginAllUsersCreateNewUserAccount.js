...
const SystemAdminAfterLoginAllUsersCreateNewUserAccount = () => {
	...
	function createNewUser(userName, firstName, lastName, password, email, role) {
		const db = startFirebase();
		const reference = ref(db, "UsersList/" + userName);
	...
	// Function for submit successful
	function submitSuccessful() {
		createNewUser(userName, firstName, lastName, password, email, role);
		navigate("/all-users"); // Direct to SystemAdminAfterLoginAllUsers.js
	}
						...
						<div className="w-full h-full pt-2 ">
							<div className="container mx-2">
								<div className="w-full max-w-full p-6 mx-auto">
									<h2 className="text-2xl text-black">New User Information</h2>
									<form className="mt-6 border-t border-gray-900 pt-4">
										<div className="flex flex-wrap -mx-3 mb-6">
											...
											<div className="w-full md:w-full px-3 mb-6">
												<label className="block uppercase tracking-wide text-black text-xs font-bold mb-2" htmlFor="grid-text-1">Last Name</label>
												<input
													className="appearance-none block w-full bg-white text-gray-700 border border-gray-400 shadow-inner rounded-md py-3 px-4 leading-tight focus:outline-none  focus:border-gray-500"
													id="grid-text-1"
											...
											<div className="flex justify-end">
												<button
													className="appearance-none bg-amber-100	 text-gray-900 px-2 py-1 ml-3 shadow-sm border hover:bg-amber-300 rounded-md mr-3"
													type="submit"
													onClick={submitSuccessful}
												>Confirm to add!
												</button>
											</div>
										</div>
...
export default SystemAdminAfterLoginAllUsersCreateNewUserAccount;

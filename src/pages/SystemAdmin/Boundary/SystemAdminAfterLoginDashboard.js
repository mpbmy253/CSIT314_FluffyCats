...
import { SystemAdminDashboardData } from "./SystemAdminDashboardData"
import { useNavigate } from "react-router-dom";

const SystemAdminAfterLoginDashboard = () => {
            ...
            <div className="flex h-full fixed">
                <div className="flex mt-20 ">
                        ...
                        <aside className="w-64" aria-label="Sidebar">
                            <div className="container overflow-y-auto py-4 px-3 bg-gray-800 h-screen">
                                ...
                                <ul className="space-y-2">
                                    <li>
                                        <button onClick={goDashBoardTab} className="flex items-center p-2 pr-28 text-base font-normal text-gray-900 rounded-lg bg-gray-100">
                                    ...
                                    <li>
                                        <butto onClick={goAllUserTab} className="flex items-center p-2 pr-32 text-base font-normal text-white rounded-lg hover:bg-gray-700">
                    ...
                    <div className="flex w-screen bg-gray-900" >
                        <div className="container px-3 py-10 ml-10">
                            <h1 className="text-4xl font-semibold text-white capitalize text-center ">Recent update news</h1>

                            <div className="grid grid-cols-2 gap-10 mt-5 justify-center">
                                {
                                    SystemAdminDashboardData.map(({ id, img_src, alt, url_link, description, date }) => (
                                        <div className="inline-flex" key={id}>
                                            <img className="object-cover w-8/12 h-60 rounded-lg"
                                                src={img_src}
                                                alt={alt}
                                            />
                                            <div className="flex flex-col justify-between py-6 mx-6">
                                                <a className="text-xl font-semibold text-gray-800 hover:underline dark:text-white "
                                                    href={url_link}>
                                                    {description}
                                                </a>
                                                <span className="text-sm text-gray-500 dark:text-gray-300">On: {date}</span>
                                            </div>
                                        </div>
                                    ))
                                }
...
export default SystemAdminAfterLoginDashboard